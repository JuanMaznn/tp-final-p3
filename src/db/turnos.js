...
export default class TurnosReservas {
  buscarTodos = async () => {
    const sql = `SELECT tr.id_turno_reserva, tr.fecha_hora, tr.valor_total, tr.atentido,
                        vp.nombres AS paciente_nombres, vp.apellido AS paciente_apellido, up.documento AS paciente_documento,
                        vm.nombres AS medico_nombres, vm.apellido AS medico_apellido, um.documento AS medico_documento
                    FROM turnos_reservas AS tr
                    INNER JOIN v_pacientes AS vp ON tr.id_paciente = vp.id_paciente
                    INNER JOIN usuarios AS up ON vp.id_usuario = up.id_usuario
                    INNER JOIN v_medicos AS vm ON tr.id_medico = vm.id_medico
                    INNER JOIN usuarios AS um ON vm.id_usuario = um.id_usuario;`;
  ...
  buscarPorId = async (idTurno) => {
    const sql = `SELECT tr.id_turno_reserva, tr.id_medico, tr.id_paciente, tr.id_obra_social,
                        tr.fecha_hora, tr.valor_total, tr.atentido, tr.activo,
                        vp.nombres AS paciente_nombres, vp.apellido AS paciente_apellido, up.documento AS paciente_documento,
                        vm.nombres AS medico_nombres, vm.apellido AS medico_apellido, um.documento AS medico_documento
                    FROM turnos_reservas AS tr
                    INNER JOIN v_pacientes AS vp ON tr.id_paciente = vp.id_paciente
